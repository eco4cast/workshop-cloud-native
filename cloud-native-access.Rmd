...
Consequently, we have to override the default "endpoint" (which assumes it will be talking to AWS), to tell it to talk to our server instead.  EFI uses a simple reverse-proxy server to provide a custom endpoint to Jetstream2, `data.ecoforecast.org`.  

Object stores use credentials to control who has permission to read and write data to the store.  

To access data on 
(Note the bucket names are global to the server)

```{r}
s3 <- s3_bucket("forecasts", endpoint_override = "data.ecoforecast.org")
```
